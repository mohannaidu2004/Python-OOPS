...
# ---------------- STREAMLIT UI ---------------- #
st.set_page_config(page_title="ATM App", page_icon="🏦")

st.title("🏦 ATM Interface with PostgreSQL")

if 'atm' not in st.session_state:
    st.session_state.atm = ATM(conn)
...
if not st.session_state.authenticated:
    ...
    if st.button("Login"):
        if st.session_state.atm.authenticate_user(username, pin):
            st.session_state.authenticated = True
            st.success(f"Welcome, {username}!")
        else:
            st.error("Invalid username or PIN.")
else:
    st.subheader(f"👋 Welcome, {st.session_state.atm.username}")
    st.info(f"💰 Current Balance: ₹{st.session_state.atm.balance:.2f}")

    option = st.radio("Select Operation", ["Deposit", "Withdraw", "Logout"])
    ...
    if option == "Deposit":
        ...
        if st.button("Withdraw Amount"):
            success = st.session_state.atm.withdraw(amount)
            if success:
...
y= int(input("emnter another number:"))
print(c.mul(x,y))
